...
- (UIImage *)imageWithImage:(UIImage *)image scaledToSize:(CGSize)newSize {
    CGSize desiredSize = newSize;
    if (newSize.height > newSize.width) {
        desiredSize.width = 250;
        desiredSize.height = desiredSize.width * (newSize.height / newSize.width);
    } else {
        desiredSize.height = 250;
        desiredSize.width = desiredSize.height * (newSize.width / newSize.height);
    }
    if ([[UIScreen mainScreen] respondsToSelector:@selector(scale)]) {
        if ([[UIScreen mainScreen] scale] == 2.0) {
            UIGraphicsBeginImageContextWithOptions(desiredSize, YES, 2.0);
        } else {
