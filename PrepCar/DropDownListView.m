...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions xy:(CGPoint)point size:(CGSize)size isMultiple:(BOOL)isMultiple
{
    ...
    CGRect rect = CGRectMake(point.x, point.y+50, size.width, height+120);
    if (self = [super initWithFrame:rect])
    {
        ...
        if (isMultipleSelection) {
            ...
            [btnDone setFrame:CGRectMake(rect.origin.x+220,rect.origin.y-100, 82, 31)];
...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions xy:(CGPoint)point size:(CGSize)size isMultiple:(BOOL)isMultiple andselected:(NSMutableArray *)selectedrows
{
    isMultipleSelection=isMultiple;
    float height = MIN(size.height, DROPDOWNVIEW_HEADER_HEIGHT+[aOptions count]*44);
    CGRect rect = CGRectMake(point.x, point.y+50, size.width, height+120);
    if (self = [super initWithFrame:rect])
    {
        ...
        if (isMultipleSelection) {
            ...
            self.arryData = [selectedrows mutableCopy];
        }
...
-(void)Click_Done{
    if (self.delegate && [self.delegate respondsToSelector:@selector(dropDown:SelectedIndexPaths:)]) {
        NSLog(@"%@",self.arryData);
        [self.delegate dropDown:self SelectedIndexPaths:self.arryData];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([self.arryData containsObject:indexPath]){
        imgarrow.frame=CGRectMake(280,2, 27, 27);
        imgarrow.image=[UIImage imageNamed:@"check_mark-2"];
	} else
        imgarrow.image=nil;
