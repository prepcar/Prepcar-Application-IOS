...
-(BOOL)checkValidation{
    ...
    if (![[AppManager sharedManager] validateEmailWithString:txtemail.text]) {
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:nil
...
-(void)contactMethod{
    ...
    NSMutableDictionary *postDictionary = [NSMutableDictionary dictionaryWithObjectsAndKeys:user1,@"user_id",txtname.text,@"name",txtemail.text,@"email",txtmessage.text,@"msg", nil];
    
    NSLog(@"%@",postDictionary);
    
    [[AppManager sharedManager] addLoadingWindow];
    NSString *strUrl=[NSString stringWithFormat:@"%@contact",kURl];
